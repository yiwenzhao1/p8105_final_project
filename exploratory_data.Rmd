...
### **Trends in Time**
#### **The Number of Crimes Over Five Years**

The figure below shows that the number of crimes in Chicago change over the most recent six years. The range of the number of crimes per year is between 4629 and 10824 from 2016 to 2021. 
...
crime_full=
  read.csv("./data/five_year_data.csv")
...
#### **The top type of crimes in Chicago over 5 years**
...
```{r, warning=FALSE, message=FALSE}
crime = read_csv("./data/five_year_data.csv") %>%
...
title( "Figure 3: Wordclouding of the top Type of Crimes Over Five Years")
```
 
 

### **Trends in Location**
#### **Top 10 Locations With High Criminal Incidence From 2017 to 2021**
```{r, warning=FALSE, message=FALSE}
## clean a new dataset with ranking of numbers and adjusted numbers
crime_loc = read.csv("./data/five_year_data.csv") %>%
...
anim_plot = staticplot + 
  ...
  labs(title = 'Figure 3: Number of Crime in Chicago from 2017 to 2021 : {closest_state}',
       subtitle = "Top 10 Location")  

animate(anim_plot, 150, fps = 15, width = 780, height = 560)
```

We also sorted and ranked the data, and compiled the top 10 locations with high criminal incidence from 2016 to 2021 into a dynamic bar graph. Through this motion chart, we can visually observe the names of the crime areas, the number of crime incidents, and their changing trends.
